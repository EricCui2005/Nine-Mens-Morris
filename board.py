...
class Board:
		...
		def __init__(self):
				...
				self._board_counts = [0, 0]
				self._placed_counts = [0, 0]
...
# Evaluate if a move is legal in phase 1
		...
		def p1_make_move(self, region, position, team, test=False):
				...
				if self.legal_move(region, position):
						...
						if team == 1 and not test:
								self._num_white_placed += 1
								self._num_white_on_board += 1
								self._board_counts[0] += 1
								self._placed_counts[0] += 1
						elif team == 2 and not test:
								self._num_black_placed += 1
								self._num_black_on_board += 1
								self._board_counts[1] += 1
								self._placed_counts[1] += 1
		...
		def phase_change(self):
				return self._placed_counts[0] == 9 and self._placed_counts[1] == 9

		#phase 2 legal moves, takes in the "coordinates" of a certain testing position to return a list of legal moves from that testing position. Returns the list in notation format
		def p2_legal_moves_notation(self, region, position):
			...
			#phase 2 legal moves, takes in the "coordinates" of a certain testing position to return a list of legal moves from that testing position. Returns the list in point format
		def p2_legal_moves(self, region, position):
				# List to store legal moves (stored as notation)
				moves = []
		...
		#is in mill. This function will evaluate whether or not a certain occupied position is part of a mill (three occupied positions in a row). It will return false for no mills and true for one or two mills (two mills is theoretically possible, but for the sake of simplification, this extremely rare instance will be ignored)
		...
		def remove_piece(self, region, position, removing_team):
				...
				if self.full_board[region][position].occupied != removing_team and self.full_board[region][position].occupied != 0:
						# remove piece
			...
			for i in range(3):
				for j in range(8):
					if self.full_board[i][j].occupied == team:
						team_pieces.append(self.full_board[i][j])

			return team_pieces

		def available_pieces_notation(self, team):
			avail = self.available_pieces(team)
			piece_strings = []
			for piece in avail:
				piece_strings.append(piece.notation)
			return piece_strings
					

	#copy board. swap teams.	return copy.
		def flip_board(self, b):
			newBoard = copy.deepcopy(b)
			
			for r in range(3):
					for p in range(8):
							if newBoard.full_board[r][p].occupied == 1 :
								newBoard.full_board[r][p].occupied = 2
							elif newBoard.full_board[r][p].occupied == 2 :
								newBoard.full_board[r][p].occupied = 1
			return newBoard


		#find all pieces	on a team in a mill
		def find_all_team_pieces_in_mills(self, team):
			pieces_in_mills=[]

			# loop eveery pieece. if on team and part of mill and not already in list, add to list. return list.
			for piece in self.full_board:
				if ((piece.occupied == team) 
					and (self.mills_at(piece.region, piece.position, self.team)>0)
						and not (piece in pieces_in_mills)):	
							pieces_in_mills.append(piece)

			return pieces_in_mills

		def piece_counts(self, team):
			return len(self.available_pieces(team))
